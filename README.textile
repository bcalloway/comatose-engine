...
# Modify your routes.rb as indicated below:
   <pre><code># If you want default routes from the plugin add this after any of your own existing routes, but before the default rails routes:
  map.from_plugin :comatose_engine  </code></pre>
   Otherwise, follow the Comatose Description for routes.
   <pre><code>#map.resources "page_photos"
   #map.comatose_admin
